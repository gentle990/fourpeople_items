<template>
    <div>
      ...
      <el-pagination background layout="prev, pager, next" :total="tableDataRef.length" :page-size="pageNumber" @current-change="handleChange" />
    </div>
  </template>
...
const tableDataRef = computed(()=>{
    return tableData.filter((item)=> item.name.includes(search.value)).filter((item)=> item.status === type.value || initType === type.value)
})
